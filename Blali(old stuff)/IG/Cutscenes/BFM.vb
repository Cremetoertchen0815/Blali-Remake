...
Namespace IG.Cutscenes
    ...
    Public Class BFM
        ...
        Public Overrides Sub Initialize()
            Config = New SceneConfig With {.ID = 123, .ReloadOnSelection = True, .AutoLoadSoundBank = False, .Descriptor = "bfm", .ShowLoadingScreen = False, .ShowLSCustom = False}
        End Sub

        Public Overrides Sub LoadContent(Optional parameter As Integer = 0)
            Cutscene = CutsceneTypeA.Load(ContentMan.RootDirectory & "\mps\bfm.cut")
            Cutscene.Init()
        ...
        Private Sub StopCutscene()
            Automator.Clear()
            MediaPlayer.Stop()
            SceneMan.ChangeToScene(New SceneSwitch(2))
        End Sub
    End Class
End Namespace
