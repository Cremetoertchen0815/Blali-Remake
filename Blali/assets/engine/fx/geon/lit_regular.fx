...
VertexShaderOutputNoFog FlatLightingMainVSNoFog(in VertexShaderInput input)
{
...
// main vertex shader for flat lighting
VertexShaderOutputFog FlatLightingMainVSFog(in VertexShaderInput input)
{
...
float4 FlatLightingMainPSNoFog(VertexShaderOutputNoFog input) : COLOR
{
...
// main pixel shader for flat lighting
float4 FlatLightingMainPSFog(VertexShaderOutputFog input) : COLOR
{
	// pixel color to return
	float4 retColor;
	...
	//apply fog
	retColor.rgb = lerp(retColor.rgb, FogColor * retColor.a, input.FogFactor);
...
technique NoFog
{
	pass P0
	{
		VertexShader = compile VS_SHADERMODEL FlatLightingMainVSNoFog();
		PixelShader = compile PS_SHADERMODEL FlatLightingMainPSNoFog();
	}
};
// default technique with flat lighting 
technique Fog
{
	pass P0
	{
		VertexShader = compile VS_SHADERMODEL FlatLightingMainVSFog();
		PixelShader = compile PS_SHADERMODEL FlatLightingMainPSFog();
	}
};