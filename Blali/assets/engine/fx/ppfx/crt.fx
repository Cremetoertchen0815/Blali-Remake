...
static const float4 scanlineTint = float4(0.8f, 0.8f, 0.8f, 0.0f);
...
float4 mainImage(float2 tex) : TARGET
{
	...
	float4 color = tex2D(SpriteTextureSampler, xy);
...
{
	return mainImage(input.TextureCoordinates) * input.Color;
}
